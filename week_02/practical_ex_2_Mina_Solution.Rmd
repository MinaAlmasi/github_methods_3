...
The data-set consists of 224 observations of 7 variables. The variables include the subject (i.e., the participant), gender (coded as F or M), scenario (what scenario the participant is in), attitude (pol = polite and inf = informal). Total duration (how long the scenario is), f0mn (pitch), hiss_count (amount of hisses made). 
...
scenario_int <- lm(f0mn~scenario, data = F1)
summary(scenario_int)
```

```{r}
#Linear model with scenario as factor 
scenario_fact <- lm(f0mn~as.factor(scenario), data = F1)
...
1.2i: In the matrix with scenario as factor it is including each level of the scenario in the design matrix as its own column whereas in the matrix with scenario as integer, a 2x2 design matrix is created (i.e., it is treating scenario as a one-level variable.)

1.2ii: We should code scenario as a factor since not doing so would leave out useful information. It is more meaningful to model the data taking each scenario into account separately. 
...
ggplot(politeness, (aes(x = scenario, y = f0mn, color = attitude)))+ 
  ...
  theme_bw()
```
Generally, it seems that all males (i.e., subjects coded as "M") have a lower pitch than females (subjects coded as "F"). 

It is difficult to see a relationship between attitude and pitch in this exploratory phase.  
...
```{r 2.1ii}
#Building a two-level model with scenario as a random effect
...
```{r 2.1iii}
#Building a two-level model with subject as a random effect
...
```{r}
#Building a two-level model with both scenario and subject as a random effect
...
The fourth model with subject and scenario as random effects has the lowest residual standard deviation and AIC value. 

Which of the second-level effects explain the most variance ?
```{r}
summary(m4)
```
The subject explains the most variance (compared to scenario).
