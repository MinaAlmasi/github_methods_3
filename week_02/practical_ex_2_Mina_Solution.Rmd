...
```{r 2.2ii}
m5 <- lm(avg_f0mn ~ gender, data = f0mn_mean)
summary(m5)
```
...
m1_qq <- ggplot(politeness, aes(sample = f0mn)) + stat_qq() +
  stat_qq_line(colour = "red") +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles") + ggtitle("Old Single Level Model (m1)") +
  theme_bw()

#new single level model
m5_qq <- ggplot(avg_f0mn, aes(sample = mean_f0mn)) + stat_qq() +
  stat_qq_line(colour = "blue") +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles") + ggtitle("New Single Level Model (m5)") +
  theme_bw()

ggpubr::ggarrange(m1_qq, m5_qq)
```
