...
The classification is not much better than chance level (50 % since it is a binary classification: it can only classify as PAS1 or PAS2)
...
Firstly, we will create a target vector with pas 1 and 4 similar to the one we made in exercise 2.1i
```{python}
pas1_4 = np.where((y==1)|(y==4))
data_1_4 = data[pas1_4]

data_1_4.shape #checking that we did it right
...
To compare between the L1 PAS 1 & 2 classification accuracy and L1 PAS 1 & 4 accuracy, we take the mean of the means:
```{python}
print("Cross-Validation PAS1-PAS2 mean of means:", round(np.mean(cross_scores_l1), 3))

print("Cross-Validation PAS1-PAS4 mean of means:", round(np.mean(cross_scores_pas1_4), 3))
```

The classification accuracy between the PAS1-PAS2 and PAS1-PAS4 is quite similar. Both are not much better than chance level (50 % since it is a binary classification but this time between PAS1 and PAS4). This is also apparent visually. 
...
Theoretically, there should be a greater difference between PAS1 and PAS4 compared to the difference between PAS1 and PAS2 considering the meaning of these ratings as a great difference in experience. However, our pairwise comparisons in 2.2 give quite similar results.
...
The mean accuracy is higher for our support vector machine with a radial basis kernel (0.333) although both accuracies are quite low (just above chance level). 
...
plt.axhline(y = 0.25, color = "black") #horisontal line at chance level 
plt.plot(times, cross_scores_Rsvc)
plt.title("SVC with Radial Basis: Classification Accuracy vs. Time")
plt.xlabel("Time (ms)")
plt.ylabel("Accuracy")
plt.show()
```
...
Looking at the plot visually, we note that classification of PAS ratings at around 200-250 is above chance level. 

We can also calculate the accuracy in the this range which also shows that the classification of subjective experience is above chance level at 200-250ms:
```{python}
## Finding the indices at the upper and lower bounds of the range 200-250 ms ##
lower = np.where(times == 200)[0][0]
upper = np.where(times == 248)[0][0]
#note that the upper boiund is 248 to make it possible to divide by 4ms 

## Mean accuracy in the range ##
print("Mean Accuracy Radial Basis in range 200-250 ms:", round(np.mean(cross_scores_Rsvc[lower:upper]),3))
```
...
X_train, X_test, y_train, y_test = train_test_split(X_all_eq_scaled, all_pas_y_eq, test_size=0.30, random_state=42) #we save to four different variables, the random_state ensures that it randomly sorts the dataset into train and test 
```
