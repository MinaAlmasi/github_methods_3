...
The mean accuracy is higher for our support vector machine with a radial basis kernel (0.333) although both accuracies are quite low (just above chance level at 25 %). 
...
#note that the upper bound is 248 to make it possible to divide by 4ms 
...
```
Performance is just above chance level
...
import pandas as pd
predicted_y = pd.Series(predicted_y, name = 'Predicted PAS-rating')
y_test = pd.Series(y_test, name = 'Actual PAS-rating')
...
We imagine that PAS 3 "Almost clear experience" is the "broadest" of the rating definitions, thereby allowing participants to classify a broader variety of experiences as PAS 3 (depending on their definition of "almost"). We could therefore expect that PAS 3 rating are more scattered across the multidimentional space, therefore making them harder to distinguish from other ratings as well as making PAS 3 the obvious/safe guess in most cases for our model, which only performs slightly better than chance. 

```{python}
#Misclassifications
(5+16+12)/(4+5+16+12)*100 #PAS1
(1+12+7)/(1+8+12+7)*100 #PAS2
(1+5+7)/(1+4+14+7)*100 #PAS3
(2+5+14)/(2+5+14+7)*100 #PAS4
