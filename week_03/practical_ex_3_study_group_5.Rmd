...
```{r 3.2}
multi_pas_interaction <- glmer(count~pas*task + (pas|subject), data = data_count, family = poisson, control = glmerControl(optimizer="bobyqa")) 

summary(multi_pas_interaction)
...
```{r}
multi_pas <- glmer(count~pas+task + (pas|subject), data = data_count, family = poisson, control = glmerControl(optimizer="bobyqa")) 
