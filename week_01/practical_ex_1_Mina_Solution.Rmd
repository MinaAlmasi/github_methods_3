...
log_fitted_values <- data.frame(wt = mtcars$wt, estimated_y = estimated_y_logm) 

ggplot(log_fitted_values, aes(wt, estimated_y)) + 
  geom_point()+
  theme_bw()+ 
...
The  __linear.predictors__ are sets of coefficients which are used to predict values of the dependent variable. In this case, there is one linear predictor *wt* = -4.023 which is used to predict the value of outcome variable *am*. The __fitted_values__ are a result of the linear predictors.
...
Interpretation of the _Intercept_: The coefficients are given in log-odds, and for interpretability, the inverse logarithm has been taken to convert the intercept to probabilities. 
...
```{r exercise 3.2ii}
#The probability of manual transmission of 
