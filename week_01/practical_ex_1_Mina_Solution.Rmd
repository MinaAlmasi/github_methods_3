...
ggplot(mtcars, aes(x=wt, y=mpg)) +
  ...
  labs(x='wt (x-values)', y='mpg (actual y-values)')
...
bhat <- solve(t(X_NEW) %*% X_NEW) %*% t(X_NEW) %*% actual_y
...
all_coefficients <- cbind(bhat, betas_q) #c-bind to combine the two vectors
colnames(all_coefficients) <- c("OLS", "lm()") #adding colnames 

all_coefficients

#Estimating the y-values (yhat) using the quadratic model 
q_estimated_y <- predict(quadratic_model)
...
```{r exercise 1.3i}
#Plotting actual vs. estimated y values
plot(actual_y, q_estimated_y)
arrows(actual_y, q_estimated_y, x1 = actual_y, y1 = actual_y, length = 0.1, angle = 3, code = 2, col = par("fg"), lty = par("lty"), lwd = par("lwd"))
title("Actual Y-values and Estimated Y-values (Quadratic Model)")


#Plotting actual y-values vs. estimated in ggplot 
intercept <- as.numeric(betas_q[1])
slope <- as.numeric(betas_q[2])

ggplot(mtcars, aes(x=wt, y=q_estimated_y)) +
  geom_point() +
  geom_abline(intercept=intercept, slope=slope) + 
  geom_linerange(aes(residuals_q), ymin = mtcars$mpg, ymax = q_estimated_y, x=mtcars$wt, color = "indianred", alpha = 0.5) + 
  labs(x='wt (x-values)', y='mpg (actual y-values)')
