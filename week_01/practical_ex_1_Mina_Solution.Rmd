...
#Plotting actual y-values vs. estimated y-values
...
quadratic_model <-  lm(formula= mpg~wt+I(wt^2), data=mtcars)

#Comparing coefficients
coefficients <- cbind(bhat, quadratic_model$coefficients) #c-bind to combine the two vectors
colnames(coefficients) <- c("OLS", "lm()") #adding colnames 
...
title("Actual Y-values and Estimated Y-values using the Quadratic Model")
```

Firstly, by looking at the "coefficients" output, we can note that the lm() function and the ordinary least squares (OLS) estimator gives exactly the same output at 6 decimals. 
