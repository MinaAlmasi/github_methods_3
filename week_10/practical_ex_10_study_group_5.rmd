...
The first 5 values stand out from the rest as their log value is below -30 whereas all other eigenvalues have a log above -10. These are the 5 noise dimensions.
...
```{python}
cov_matrix = np.cov(W, rowvar = False) #rowvar = False is specified to choose the sensor dimension
...
**ATTENCIONE** WRITE NOTES MY DUDES
...
```
**ATTENCIONE** we think that it is W_dxk instead of Z_dxk...........-.
...
The proportion correctly classified completely stagnates with the last 5 principle components. They essentially do not make the model better. We believe this is due to the fact that they are the noise dimensions hence it would make sense that they do not improve the model.
...
plt.show()
```
The plot is different in the sense that the proportion correctly classified does not just increase with the number of principle components. This is because that when we do cross-validation, we punish overfitting as we work with both training and test data. 
...
```{python}
print("Difference:", round(cv_scores[peak_pc] - cv_scores[-1], 3) * 100, "Percentage Points")
```
...
***ATTENCIONE*** the dudes will answer this       question        in the future :DDDD
