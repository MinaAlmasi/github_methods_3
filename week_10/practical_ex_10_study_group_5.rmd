...
Firstly, we run the PCA with the k-max principle components with 16 which we found to be the optimal amount of principle components: 
...
In general, it seems that the PCA-reduced model has higher accuracies in the given interval. The best performance of the PCA-reduced model is seen at 248 ms. This makes sense, since our choice of 16 PCs is based on logistic regression at this particular time. 

In the interval around the peak magnetic activity, 248 ms, a signal/effect is present (that is the case for sensor 73 but here we assume this pattern applies to all sensors). This motivates the use of an underlying data generator (i.e., principal components). Therefore, the PCA-reduced model performs better around that time.  
