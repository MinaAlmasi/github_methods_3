...
The _correct_ variable was also converted to a factor as it is binary data coded as 0 for incorrect and 1 for a correct answer. 
...
_target.contrast_ is the contrast between the target and the background. In experiment 1, it is held constant across participants whereas each participant had their own _target.contrast_ in experiment 2.

_target.frames_ is the amount of frames in which the target is shown (11.8 ms pr frame). In experiment 2, it was held constant at 3 frames. On the other hand, the amount of frames varies from 1 to 6 in the current dataset from experiment 1. _target.frames_ can be considered a type of time variable as it describes the duration the target was shown. It is thus a ratio variable as the ratio of two measurements has a meaningful interpretation. That is, 4 _target.frames_ (47.2 ms) is twice as long as 2 _target.frames_ (23.6 ms). For this reason, its class is coded as an integer: 

**ATTENCIONE CHECK TEXT ABOVE BEFORE FINAL KNIT** 
```{r}
...
In the 'lower' argument, we set a = 0 because the lowest possible accuracy is 0 and b = 0.5 because the best accuracy should not be below chance. (**ATTENCTIONE Lau vil gerne sørge for at den ikke er lavere end 50 %, så det er lol at vi sætter a = 0, så jeg har sat den til 0.5 - det kan vi snakke om i morgen**)
...
```{r 7.1i}
par1 <- optim(par = c(0.5,1,1,1), fn=RSS, data = filter(subject7, pas == "1"), method = 'L-BFGS-B', lower = c(0.5,0.5,-Inf,-Inf), upper = c(1,1,Inf,Inf))

par2 <- optim(par = c(0.5,1,1,1), fn=RSS, data = filter(subject7, pas == "2"), method = 'L-BFGS-B', lower = c(0.5,0.5,-Inf,-Inf), upper = c(1,1,Inf,Inf))

par3 <- optim(par = c(0.5,1,1,1), fn=RSS, data = filter(subject7, pas == "3"), method = 'L-BFGS-B', lower = c(0.5,0.5,-Inf,-Inf), upper = c(1,1,Inf,Inf))

par4 <- optim(par = c(0.5,1,1,1), fn=RSS, data = filter(subject7, pas == "4"), method = 'L-BFGS-B', lower = c(0.5,0.5,-Inf,-Inf), upper = c(1,1,Inf,Inf))
```

##### ii. Plot the fits for the PAS ratings on a single plot (for subject 7) `xlim=c(0, 8)`

Firstly, we need to calculate the estimated y-values for subject one using the estimates we have been given by the optim function
```{r 7ii yhats}
#calculating the yhats using the formula a + ((b - a)/(1+exp((c - x)/d)))
newdata <- data.frame(cbind("x" = seq(0, 8, by = 0.01)))

newdata$yhat1 <- par1$par[1] + ((par1$par[2]-par1$par[1])/(1+exp((par1$par[3]-newdata$x)/par1$par[4])))
newdata$yhat2 <- par2$par[1] + ((par2$par[2]-par2$par[1])/(1+exp((par2$par[3]-newdata$x)/par2$par[4])))
newdata$yhat3 <- par3$par[1] + ((par3$par[2]-par3$par[1])/(1+exp((par3$par[3]-newdata$x)/par3$par[4])))
newdata$yhat4 <- par4$par[1] + ((par4$par[2]-par4$par[1])/(1+exp((par4$par[3]-newdata$x)/par4$par[4])))
```

Now we are ready to plot the values:
```{r 7ii plotting}
ggplot(newdata) + 
  geom_line(aes(x=x, y=yhat1, color = "pas1"))+
  geom_line(aes(x=x, y=yhat2, color = "pas2"))+
  geom_line(aes(x=x, y=yhat3, color = "pas3"))+
  geom_line(aes(x=x, y=yhat4, color = "pas4"))+
  scale_color_manual(values = c("pas1" = "#F8766D", "pas2" = "#7CAE00", "pas3" = "#00BFC4", "pas4" = "#C77CFF"))+
  xlim(c(0,8))+
  ylim(c(0,1))+
  labs(y = "Predicted Correct", x = "Target Frames", title = "Plotting the Sigmoid Fits (Optim)")+
  theme_bw()+
...
Firstly, we need to create the data and calculate the predicted values. We define the x-values using the seq function (to create an xlim of 0 to 8). We have to specify the sequencing with the by argument which is calculated as such: $$((\text{to}-\text{from})/(\text{length.out}-1))$$
```{r}
newdata2 <- data.frame(cbind('target.frames' = seq(0, 8, by = 0.001), "pas" = rep(1:4), subject = rep("7")))
```
...
newdata2$yhat <- predict(m7, newdata = newdata2, type = "response") # type response defined to predict responses back to 0 target frames albeit no real data is present. 
```
...
ggplot(newdata2) + 
  ...
  xlim(c(0,8))+
  ylim(c(0,1))+
  labs(y = "Predicted Correct", x = "Target Frames", title = "Model from 6.1")+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 15))
```
