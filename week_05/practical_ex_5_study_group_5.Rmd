...
pacman::p_load(lme4, tidyverse, readbulk, boot)
```
...
intercept <- inv.logit(m4@beta[1])
slope <- inv.logit(m4@beta[2])

ggplot(data, aes(x = target.frames, y = as.numeric(as.character(correct))))+
  geom_point(aes(y=as.numeric(as.character(correct))), color = "black")+
  xlim(min = 0, max = 8)+
