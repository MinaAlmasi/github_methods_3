...
Firstly, we need to create the data and calculate the predicted values
```{r}
data$fitted <- fitted(m7) #calculating all fitted values

subject7_new <- data %>% 
  filter(subject == 7) %>% #filtering so we only get the fitted values for subject 7 
  dplyr::select(pas, fitted, target.frames)
```

To begin with, we create the dataset for pas 1.. We define the x-values using the seq function (to create an xlim of 0 to 8). We have to specify the sequencing with the by argument which is calculated as such: $$((\text{to}-\text{from})/(\text{length.out}-1))$$
```{r}
pas1 <- subject7_new %>%
  filter(pas == "1") %>% 
  dplyr::select(fitted) %>% 
  rename("yhat1" = fitted) %>% 
  mutate("x" = cbind("x" = seq(0, 8, by = 0.04395)))
```
...
pas2 <- subject7_new %>%
  filter(pas == "2") %>% 
  rename("yhat2" = fitted) %>% 
  mutate("x" = cbind("x" = seq(0, 8, by = 0.0620155)))

pas3 <- subject7_new %>%
  filter(pas == "3") %>% 
  rename("yhat3" = fitted) %>% 
  mutate("x" = cbind("x" = seq(0, 8, by = 0.1038961)))

pas4 <- subject7_new %>%
  filter(pas == "4") %>% 
  rename("yhat4" = fitted) %>% 
  mutate("x" = cbind("x" = seq(0, 8, by = 0.01694915)))
```

Now we can plot it: 
```{r}
ggplot() + 
  geom_line(data = pas1, aes(x=x, y=yhat1), color = "red")+
  geom_line(data = pas2, aes(x=x, y=yhat2), color = "green")+
  geom_line(data = pas3, aes(x=x, y=yhat3), color = "blue")+
  geom_line(data = pas4, aes(x=x, y=yhat4), color = "purple")+
  xlim(c(0,8))+
  ylim(c(0,1))+
  ylab("Predicted Correct")+
  xlab("Target Frames")+
