---
title: "Portfolio Assignment 1, Study Group 5, Methods 3, 2021, autumn semester"
author:
...
date: "First edit: 22/09/21, Final edit: 13/12/21"
output:
  html_document:
    df_print: paged
---
...
### 1) (MS) Describe the dataset, such that someone who happened upon this dataset could understand the variables and what they contain  
  
#### (MS) i. Also consider whether any of the variables in _politeness_ should be encoded as factors or have the factor encoding removed. Hint: ```?factor```  
```{r 1.1}
...
Gender, attitude, subject and scenario should all be turned into factors since they are categorical variables. *gender* and *attitude* are binary variables whereas *scenario* and *subject* have several levels.  
...
### 2) (MA) Create a new data frame that just contains the subject _F1_ and run two linear models; one that expresses _f0mn_ as dependent on _scenario_ as an integer; and one that expresses _f0mn_ as dependent on _scenario_ encoded as a factor
...
#### (MA) i. Include the model matrices, $X$ from the General Linear Model, for these two models in your report and describe the different interpretations of _scenario_ that these entail
```{r 1.2i}
...
This has consequences for how the model treats *scenario*. As an integer, the model considers scenario 2 to be "worth" twice as much as scenario 1. The model can be visualized as a line in a two-dimensional plot. 

As a factor, each scenario has an estimate for itself. That is, each scenario has its distinct effect on f0mn.

#### (MA) ii. Which coding of _scenario_, as a factor or not, is more fitting?
...
### 3) (DB) Make a plot that includes a subplot for each subject that has _scenario_ on the x-axis and _f0mn_ on the y-axis and where points are colour coded according to _attitude_
```{r 1.3}
...
#### (DB) i. Describe the differences between subjects
...
##### (MA) i. a single level model that models _f0mn_ as dependent on _gender_
```{r 2.1i}
m1 <- lm(f0mn~gender, data = politeness)
```

##### (MS) ii. a two-level model that adds a second level on top of i. where unique intercepts are modelled for each _scenario_
```{r 2.1ii}
m2 <- lmer(f0mn~gender+(1|scenario), data = politeness)
```

##### (DB) iii. a two-level model that only has _subject_ as an intercept 
```{r 2.1iii}
m3 <- lmer(f0mn~gender+(1|subject), data = politeness)
```

##### (ADS) iv. a two-level model that models intercepts for both _scenario_ and _subject_
```{r 2.1iv}
m4 <- lmer(f0mn~gender+(1|subject)+(1|scenario), data = politeness)
```

##### (ADS) v. which of the models has the lowest residual standard deviation, also compare the Akaike Information Criterion `AIC`? 
```{r 2.1v}
#Calculating the residual standard deviation using the sigma() function
residual_sd <- tibble("m1" = sigma(m1), "m2" = sigma(m2), "m3"= sigma(m3), "m4" = sigma(m4))
...
The residual standard deviation penalizes more complex models as the formula includes degrees of freedom, $df$: 

$$ \text {residual standard deviation} = \sqrt{\frac{\sum^n_{i = 1}\epsilon^2_i}{df}}$$
Where the degrees of freedom $df$ is calculated by subtracting the model parameters from the total amount of observations $n$. Complex models with more parameters will thus have a lower $df$ which in turn lead to a higher residual standard deviation. 

Likewise, the AIC value also penalizes more complex models. Despite this, the fourth model (m4) with subject and scenario as random effects has the lowest residual standard deviation as well as the lowest AIC. 


##### (ADS) vi. which of the second-level effects explains the most variance?
```{r 2.1vi}
summary(m4)
```

Looking at the output for random effects, we see that including random intercepts for *subject* accounts for a higher amount of variance (588.83) compared to the random intercepts for *scenario* (96.17). 

#### 2) Why is our single-level model bad?
##### (DB) i. create a new data frame that has three variables, _subject_, _gender_ and _f0mn_, where _f0mn_ is the average of all responses of each subject, i.e. averaging across _attitude_ and _scenario_
```{r 2.2i}
avg_f0mn <- politeness %>% 
...
##### (DB) ii. build a single-level model that models _f0mn_ as dependent on _gender_ using this new dataset
```{r 2.2ii}
m5 <- lm(mean_f0mn ~ gender, data = avg_f0mn)
```

  
##### (MS) iii. make Quantile-Quantile plots, comparing theoretical quantiles to the sample quantiles) using `qqnorm` and `qqline` for the new single-level model and compare it to the old single-level model (from 1).i). Which model's residuals ($\epsilon$) fulfill the assumptions of the General Linear Model better?)
```{r 2.2iii}
par(mfrow = c(1,2))
...
It seems that the new model (*m5*) fulfills the assumptions of the linear model better.  The points fall closer to the line and the deviation from the line are fairly unsystematic. For the old model (*m1*), there are several points in both ends of the plot which deviate radically from the line. By averaging the original data (which seems rather not normally distributed), we approach normality which follows the central limit theorem. This is, however at the cost of information loss. 

##### (ADS) iv. Also make a quantile-quantile plot for the residuals of the  multilevel model with two intercepts. Does it look alright?
```{r 2.2iv}
par(mfrow = c(1,1))
...
##### (MA) i. Create a plot for each subject, (similar to part 3 in Exercise 1), this time also indicating the fitted value for each of the subjects for each for the scenarios (hint use `fixef` to get the "grand effects" for each gender and `ranef` to get the subject- and scenario-specific effects)
```{r 2.3}
fitted <- fitted(m4)
...
In the plot above, the grey dots are the fitted values for each of the subjects for each of the scenarios. 
    
## Exercise 3 - now with attitude

### 1) Carry on with the model with the two unique intercepts fitted (_scenario_ and _subject_).

#### (MA) i. now build a model that has _attitude_ as a main effect besides _gender_
```{r 3.1i}
m6 <- lmer(f0mn ~ gender + attitude + (1|subject) + (1|scenario), data = politeness)
```

#### (MS) ii. make a separate model that besides the main effects of _attitude_ and _gender_ also include their interaction
```{r 3.1ii}
m7 <- lmer(f0mn ~ gender * attitude + (1|subject) + (1|scenario), data = politeness)
```


#### (DB) iii. describe what the interaction term in the model says about Korean men's pitch when they are polite relative to Korean women's pitch when they are polite (you don't have to judge whether it is interesting) 
...
#### (ADS) 2) Compare the three models (1. gender as a main effect; 2. gender and attitude as main effects; 3. gender and attitude as main effects and the interaction between them. For all three models model unique intercepts for _subject_ and _scenario_) using residual variance, residual standard deviation and AIC.  
...
m7 has the lowest AIC and the lowest residual variance. m6 has the lowest residual standard deviation, and is quite close to m7 on the other two parameters. m7 is the most complex model, since it has an interaction term, and the AIC & residual sd are supposed to penalise m7 for this complexity. We choose m7 for further analysis although m6 could also be justified as it has very similar values. 

#### 3) (GROUP) Choose the model that you think describe the data the best - and write a short report on the main findings based on this model. At least include the following:
...
```{r}
#(ADS) for an output with p-values 
m7 <- lmerTest::lmer(f0mn ~ gender * attitude + (1|subject) + (1|scenario), data = politeness)
summary(m7)
```
(GROUP) The data set consists of records of pitch levels in response to polite and informal statements by Koreans. Overall, there are 224 observations of 7 variables which include:
i. subject (i.e., the participant)
ii. gender (coded as F = female or M = male)
iii. scenario (which of the 7 scenarios the participant was in), 
iv. attitude (pol = polite & inf = informal) 
...
(DB) In the analysis, we investigated the effect of gender and attitude on pitch. To test this effect, we fitted several linear mixed-effects models with pitch (*f0mn*) as the outcome variable and other variables as fixed and random effects. Our final model (m7) has pitch as the outcome variable, *gender* and *attitude* as fixed effects with *gender* and *attitude* as an interaction effect. The model includes random intercepts for subjects and scenarios. 

(MS) This decision is made due to the fact that the study was conducted on 16 participants, 9 females and 7 males and we expect these participants to perform differently compared to each other (i.e., they will have different baselines.) There are 7 different scenarios (tasks) in the study, and since the scenarios are different, we also expect different baselines for scenarios. 

(MA) Males have a significantly lower pitch than females ($\beta = -118.232$, $t(17.158)=-8.738$, $p<.05$) when attitude is informal. Pitch also significantly decreased when subjects spoke politely compared to informally, ($\beta = -17.192$, $t(188.445)=-3.170$, $p<.05$). The interaction term was not significant. It should be noted that p-values for mixed effects modeling are calculated slightly differently than a general linear model since there is no clear consensus about how to deal with the degrees of freedom in a mixed effects model. 

(ADS) The two random intercepts on the second level do not explain the same amount of variance. *subject* accounts for 584.4 (SD = 24.17) whereas *scenario* only accounts for 106.4 (SD = 10.32). Hence we account for much more of the unexplained variance by adding subject as a random intercept compared to scenario. This is also what we would expect considering the differences between subjects’ pitches logically should be larger than the difference in pitch as a consequence of the task.
