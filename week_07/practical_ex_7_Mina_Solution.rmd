...
```{python}
samples = [np.random.normal(loc = 0, scale = 5, size = len(y_true)) for i in range(100)]
```
...
#Linear Fits 
linear_fits = [regressor.fit(x_reshape, y) for y in samples]

#Quadratic Fits 
quadratic = PolynomialFeatures(degree=2)
X_quadratic = quadratic.fit_transform(x.reshape(-1, 1))

quadratic_fits = [regressor.fit(X_quadratic, y) for y in samples]
...
fifth_order_fits = quadratic_fits = [regressor.fit(X_fifth_order, y) for y in samples]
```
...
```{r}
plt.figure()
[plt.plot(x_reshape, y) for y in linear_fits] #linear
